% 第一步：实现直线型区域增长算法
%该命令实现的是轮廓点的区域增长算法，在增长的过程中，利用最小二乘进行空间直接拟合，并依据点到直线距离进行增长的判断
%输入：轮廓点数据：input_pnts(nx3)，每次区域增长的数量number_neighbor（大概是5左右）,距离分辨率，可以用range_resolut命令得到
%输出：得到增长后不同类点云segment（元包形式），pcData 所有数据为1显示都增长完了
function [pcData,segment] = Line_region_growing_LS(input_pnts,number_neighbor,range_resolution)
       ...
       %先计算角度是否满足阈值
        current_dis=sqrt(sum((current_neighbor(:,1:3)-current_seed(:,2:4)).^2,2)); %计算选取的领域点和当前种子点的距离
         if size(cluster,1)>1
             ...
             [parameter1] = space_line_LS(tempor_cluster);
             [PL_dis1] = PL_distance_LS(tempor_cluster,parameter1);
             mean_PLdis=mean(PL_dis1);
             [parameter2] = space_line_LS(cluster);
             [PL_dis2] = PL_distance_LS(current_neighbor(:,1:3),parameter2);
         else
            mean_PLdis=0;
            PL_dis2=0;
         end
